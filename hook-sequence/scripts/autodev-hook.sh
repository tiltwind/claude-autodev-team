...
if [ ! -f "$PIPELINE" ]; then
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] [autodev] Error: Pipeline script not found: ${PIPELINE}" >&2
  exit 1
fi
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] [autodev] Pipeline triggered in background (PID: $!). Logs: ${AUTODEV_DIR}/pipeline.log"
